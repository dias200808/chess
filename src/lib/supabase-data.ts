...
export function mapProfile(row: Record<string, unknown>): UserProfile {
  return {
    ...
    role: row.role ? (String(row.role) as UserProfile["role"]) : "student",
    schoolName: row.school_name ? String(row.school_name) : undefined,
    age: row.age === null || row.age === undefined ? null : Number(row.age),
    teacherVerification: row.teacher_verification
      ? (String(row.teacher_verification) as UserProfile["teacherVerification"])
      : "unverified",
...
function profileToRow(profile: Partial<UserProfile>) {
  return {
    ...
    age: profile.age,
    teacher_verification: profile.teacherVerification,
    city: profile.city,
...
export function mapGame(row: Record<string, unknown>): SavedGame {
  return {
    ...
    whiteGuestKey: row.white_guest_key ? String(row.white_guest_key) : undefined,
    blackGuestKey: row.black_guest_key ? String(row.black_guest_key) : undefined,
    whitePlayer: String(row.white_player ?? "White"),
    blackPlayer: String(row.black_player ?? "Black"),
    mode: String(row.mode ?? "local") as GameMode,
    matchType: row.match_type ? String(row.match_type) as SavedGame["matchType"] : undefined,
    ...
    gameType: row.game_type ? String(row.game_type) as SavedGame["gameType"] : undefined,
    ...
    whiteRatingBefore: row.white_rating_before === null || row.white_rating_before === undefined ? undefined : Number(row.white_rating_before),
    whiteRatingAfter: row.white_rating_after === null || row.white_rating_after === undefined ? undefined : Number(row.white_rating_after),
    whiteRatingChange: row.white_rating_change === null || row.white_rating_change === undefined ? undefined : Number(row.white_rating_change),
    blackRatingBefore: row.black_rating_before === null || row.black_rating_before === undefined ? undefined : Number(row.black_rating_before),
    blackRatingAfter: row.black_rating_after === null || row.black_rating_after === undefined ? undefined : Number(row.black_rating_after),
    blackRatingChange: row.black_rating_change === null || row.black_rating_change === undefined ? undefined : Number(row.black_rating_change),
    ...
    .upsert(
      {
        ...
        white_guest_key: game.whiteGuestKey ?? null,
        black_guest_key: game.blackGuestKey ?? null,
        white_player: game.whitePlayer,
        black_player: game.blackPlayer,
        mode: game.mode,
        match_type: game.matchType ?? null,
        result: game.result,
        winner: game.winner,
        ...
        white_rating_before: game.whiteRatingBefore ?? null,
        white_rating_after: game.whiteRatingAfter ?? null,
        white_rating_change: game.whiteRatingChange ?? null,
        black_rating_before: game.blackRatingBefore ?? null,
        black_rating_after: game.blackRatingAfter ?? null,
        black_rating_change: game.blackRatingChange ?? null,
...
export async function createGuestRoomInSupabase(timeControl = "10-0", hostKey?: string, hostRating?: number | null) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      white_player: "Guest",
      black_player: "Guest",
      white_player_type: "guest",
      black_player_type: "guest",
      white_rating: hostRating ?? null,
...
export function mapRoom(row: Record<string, unknown>): Room {
  return {
    ...
    whitePlayer: String(row.white_player ?? "White"),
    blackPlayer: String(row.black_player ?? "Black"),
    whitePlayerType: (String(row.white_player_type ?? "guest") as PlayerKind),
    blackPlayerType: (String(row.black_player_type ?? "guest") as PlayerKind),
    whiteRating: row.white_rating === null || row.white_rating === undefined ? undefined : Number(row.white_rating),
    blackRating: row.black_rating === null || row.black_rating === undefined ? undefined : Number(row.black_rating),
    ...
    gameType: String(row.game_type ?? (row.rated ? "rated" : "casual")) as OnlineGameType,
    ...
    connectDeadlineAt: row.connect_deadline_at ? String(row.connect_deadline_at) : undefined,
    firstMoveDeadlineAt: row.first_move_deadline_at ? String(row.first_move_deadline_at) : undefined,
    ...
    drawOfferPly:
      row.draw_offer_ply === null || row.draw_offer_ply === undefined ? undefined : Number(row.draw_offer_ply),
    whiteDrawBlockedUntilPly:
      row.white_draw_blocked_until_ply === null || row.white_draw_blocked_until_ply === undefined
        ? undefined
        : Number(row.white_draw_blocked_until_ply),
    blackDrawBlockedUntilPly:
      row.black_draw_blocked_until_ply === null || row.black_draw_blocked_until_ply === undefined
        ? undefined
        : Number(row.black_draw_blocked_until_ply),
    rematchRequestedBy: row.rematch_requested_by ? (String(row.rematch_requested_by) as Room["rematchRequestedBy"]) : undefined,
    rematchRoomId: row.rematch_room_id ? String(row.rematch_room_id) : undefined,
